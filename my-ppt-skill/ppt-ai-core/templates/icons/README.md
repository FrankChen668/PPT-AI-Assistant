# SVG Icon Library

This directory provides **6000+ high-quality SVG icons** from [Tabler Icons](https://tabler.io/icons) that can be directly embedded into SVG files generated by PPT Master.

## Libraries

| Library | Style | Count | viewBox | Prefix |
|---------|-------|-------|---------|--------|
| `tabler-filled` | fill / solid | 1000+ | `0 0 24 24` | `tabler-filled/` |
| `tabler-outline` | stroke / line | 5000+ | `0 0 24 24` | `tabler-outline/` |

> The former `chunk/` compatibility icon set is not shipped in the Public Alpha asset set because its per-icon upstream licenses could not be established from the checked-in files. Use the Tabler libraries above.

---

## Usage

Use placeholder syntax **during SVG generation**:

```xml
<!-- tabler-filled (solid style) -->
<use data-icon="tabler-filled/home" x="100" y="200" width="48" height="48" fill="#0076A8"/>

<!-- tabler-outline (line style) -->
<use data-icon="tabler-outline/home" x="100" y="200" width="48" height="48" fill="#0076A8"/>
```

**Attributes**:
- `data-icon` — `<library>/<icon-name>` (filename without `.svg`)
- `x`, `y` — Position
- `width`, `height` — Size (base 24px; `width=48` → `scale(2)`)
- `fill` — Color

`finalize_svg.py` auto-embeds all placeholders during post-processing. To run manually:

```bash
python3 scripts/svg_finalize/embed_icons.py svg_output/*.svg
```

---

## Searching for Icons

Use `ls | grep` — zero token cost:

```bash
ls skills/ppt-master/templates/icons/tabler-filled/ | grep home
ls skills/ppt-master/templates/icons/tabler-outline/ | grep chart
```

---

## Style Rules

- **Fill / Solid**: use `tabler-filled`
- **Stroke / Line**: use `tabler-outline`
- ⚠️ Do **not** mix fill and outline icons in the same presentation
