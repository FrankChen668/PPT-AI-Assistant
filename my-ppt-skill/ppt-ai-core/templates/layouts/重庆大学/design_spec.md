...
**Layout Structure**:
- Upper-right area: Optional user-provided logo placeholder (`{{LOGO}}`)
...
| Placeholder | Applicable Context | Notes |
|------------|-------------------|-------|
| `{{LOGO}}` | Any background | Supply an authorized logo or leave as text |
...
5. Use an authorized user-provided logo only when replacing `{{LOGO}}`
