...
def get_heading_level(size: float, size_map: dict, text: str = "", flags: int = 0, strict: bool = True) -> int:
    """
...
def detect_headers_footers(doc: fitz.Document, threshold_ratio: float = 0.6) -> set[str]:
    ...
    if len(doc) > HEADER_FOOTER_SAMPLE_LIMIT:
        pages_to_scan = pages_to_scan[:HEADER_FOOTER_EDGE_SAMPLE_SIZE] + pages_to_scan[-HEADER_FOOTER_EDGE_SAMPLE_SIZE:]
...
def extract_pdf_to_markdown(pdf_path: str, output_path: str = None) -> str:
    ...
    print(
        f"   Font size mapping: body={size_map.get('body', 'N/A')}, "
    ...
    for page_num, page in enumerate(doc, 1):
        ...
        for tab in tabs:
            page_elements.append({"y0": tab.bbox[1], "type": 2, "content": tab.to_markdown()})
            ...
            if el["type"] == 0 and not el.get("is_heading") and not el.get("is_list"):
                ...
                while j < len(page_elements):
                    ...
                    if not should_merge_lines(
                        {"content": merged_content, "is_heading": False, "is_list": False}, next_el
                    ):
                        break
                    merged_content += " " + next_el["content"]
                    j += 1
                merged_elements.append(
                    {"type": 0, "content": remove_page_footer(merged_content), "is_heading": False, "is_list": False}
                )
                i = j
            else:
