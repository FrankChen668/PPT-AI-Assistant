...
def print_results(results: list[ImageAnalysis]) -> None:
    ...
    for i, img in enumerate(results, 1):
        print(
            ...
            f"{img['aspect_ratio']:<7.2f} {img['filesize_kb']:<10.1f}KB "
            f"{img['layout_hint']:<15} {img['filename'][:40]}"
        )
        ...
        if imgs:
            ...
            for img in imgs[:5]:  # Show only the first 5
                print(
                    ...
                    f"(ratio {img['aspect_ratio']:.2f}) - {img['filename'][:35]}..."
                )
            if len(imgs) > 5:
...
def generate_markdown(results: list[ImageAnalysis]) -> None:
    ...
    for img in results:
        ...
        print(
            ...
            f"{img['aspect_ratio']:.2f} | {layout_desc} | (to be filled) | Existing | - |"
        )
    print("\\n" + "=" * REPORT_WIDTH + "\\n")
