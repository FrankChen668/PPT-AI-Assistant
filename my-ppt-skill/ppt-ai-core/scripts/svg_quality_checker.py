...
class SVGQualityChecker:
    ...
    def _check_text_elements(self, content: str, result: Dict):
        ...
        if text_matches:
            result['warnings'].append(
                f"Detected {len(text_matches)} potentially overly long single-line "
                "text(s) (consider using tspan for wrapping)"
            )
    ...
    def print_summary(self):
        ...
        if self.summary['errors'] > 0 or self.summary['warnings'] > 0:
            print("\n[TIP] Common fixes:")
            print("  1. viewBox issues: Ensure consistency with canvas format (see references/canvas-formats.md)")
            print("  2. foreignObject: Use <text> + <tspan> for manual line breaks")
            print("  3. Font issues: Use system UI font stack")
