        ...
        for item in source_items:
            ...
            if suffix in {".md", ".markdown"}:
                ...
                if duplicate_markdown is not None:
                    ...
                    summary["notes"].append(
                        f"{item}: skipped duplicate markdown import because equivalent "
                        f"content already exists as {duplicate_markdown.name}"
                    )
                    continue
            ...
            if suffix in PDF_SUFFIXES:
                ...
                if canonical_markdown_path.exists():
                    summary["markdown"].append(str(canonical_markdown_path))
                    summary["notes"].append(
                        f"{item}: skipped document auto-conversion because "
                        f"{canonical_markdown_path.name} already exists"
                    )
                    continue
                markdown_path = canonical_markdown_path
                try:
