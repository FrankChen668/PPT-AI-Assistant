...
Recommended first choice for native PDFs. PDF input requires the optional dependency bundle:

```bash
pip install -r scripts/requirements-pdf.txt -c scripts/constraints.txt
```
