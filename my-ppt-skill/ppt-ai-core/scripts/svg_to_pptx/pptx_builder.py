...
from .drawingml_converter import convert_svg_to_slide_shapes, scan_svg_source_stats
from .native_conversion_report import (
    REPORT_FILENAME,
    build_native_conversion_report,
    conversion_issue,
    empty_slide_stats,
    finalize_slide_stats,
    write_native_conversion_report,
)
...
def _make_writable_temp_dir() -> Path:
    base = Path(tempfile.gettempdir())
    for _ in range(10):
        candidate = base / f"svg_to_pptx_{uuid.uuid4().hex}"
        try:
...
def create_pptx_with_native_svg(
    ...
    Args:
        ...
        conversion_report_path: Structured native conversion report path.

    Returns:
        Whether the package was created. Native conversion loss is classified
        in the structured report instead of being hidden in the process code.
    """
    if not svg_files:
    ...
    if not use_native_shapes and use_compat_mode and renderer_name is None:
        print("Warning: No PNG renderer available, cannot use compatibility mode")
    ...
    temp_dir = _make_writable_temp_dir()
    ...
    try:
        ...
        success_count = 0
        has_any_image = False
        slide_conversion_stats: list[dict[str, object]] = []
        ...
        for i, svg_path in enumerate(svg_files, 1):
            ...
            try:
                # ---- Native shapes mode ----
                if use_native_shapes:
                    slide_xml, media_files_dict, rel_entries, conversion_stats = convert_svg_to_slide_shapes(
                        svg_path, slide_num=slide_num, verbose=verbose,
                    )
                    slide_conversion_stats.append(conversion_stats)
...
<Relationships xmlns="http://schemas.openxmlformats.org/package/2006/relationships">
  ...
  />{extra_rels}
                ...
                if use_native_shapes:
                    try:
                        failed_stats = scan_svg_source_stats(svg_path, slide_num)
                    except Exception:
                        failed_stats = empty_slide_stats(f'slide_{slide_num:02d}', svg_path.name)
                    failed_stats['blank_slide'] = True
                    failed_stats['conversion_errors'] = 1
                    failed_stats['issues'].append(
                        conversion_issue(
                            slide_id=f'slide_{slide_num:02d}',
                            tag='svg',
                            element_id=None,
                            error_type='slide-conversion-exception',
                            message=str(e),
                            hard_blocker=True,
                            code='native-conversion-failed',
                        )
                    )
                    slide_conversion_stats.append(finalize_slide_stats(failed_stats))
                if verbose:
        ...
        if use_native_shapes:
            with zipfile.ZipFile(output_path, 'r') as zf:
                pptx_slide_count = sum(
                    1
                    for name in zf.namelist()
                    if re.fullmatch(r'ppt/slides/slide\d+\.xml', name.replace('\\', '/'))
                )
            report = build_native_conversion_report(
                slide_conversion_stats,
                source_slide_count=len(svg_files),
                pptx_slide_count=pptx_slide_count,
            )
            report_path = conversion_report_path or output_path.parent / REPORT_FILENAME
            write_native_conversion_report(report_path, report)
            if verbose:
                print(f"  Native conversion report: {report_path}")
        ...
        if verbose:
            ...
            if use_compat_mode and has_any_image:
                ...
                if renderer_name == 'svglib' and renderer_hint:
