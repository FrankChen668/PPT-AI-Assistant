...
def _run_style_key(run: dict[str, Any]) -> tuple[Any, ...]:
    return (
        ...
        run.get('font_family'),
        run.get('font_style'),
        run.get('text_decoration'),
        run.get('opacity'),
    )


def _merge_run_char(line: list[dict[str, Any]], run: dict[str, Any], ch: str) -> None:
    if not line:
        line.append({**run, 'text': ch})
        return
    if _run_style_key(line[-1]) == _run_style_key(run):
        line[-1]['text'] += ch
    else:
        line.append({**run, 'text': ch})


def _line_visual_width(line_runs: list[dict[str, Any]]) -> float:
    width = 0.0
    for run in line_runs:
        text = run.get('text', '')
        if not text:
            continue
        fs = float(run.get('font_size', 16))
        fw = str(run.get('font_weight', '400'))
        width += estimate_text_width(text, fs, fw)
    return width


def _trim_line_right_spaces(line_runs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    if not line_runs:
        return line_runs
    while line_runs:
        text = line_runs[-1].get('text', '')
        trimmed = text.rstrip()
        if trimmed == text:
            break
        if trimmed:
            line_runs[-1]['text'] = trimmed
            break
        line_runs.pop()
    return line_runs


def _wrap_runs_by_width(
    runs: list[dict[str, Any]],
    max_width: float,
    ...
    for run in runs:
        ...
        for ch in text:
            if ch == '\n':
                current = _trim_line_right_spaces(current)
                if current:
            ...
            ch_width = estimate_text_width(ch, fs, fw)
            overflow = current and (current_width + ch_width > max_width)
            if overflow:
                current = _trim_line_right_spaces(current)
                if current:
                    lines.append(current)
            ...
            _merge_run_char(current, run, ch)
    ...
    return lines or [[{**runs[0], 'text': runs[0].get('text', '')}]]
...
def _max_text_width(
    ...
    explicit = elem.get('data-max-width') or elem.get('max-width') or elem.get('width')
    if explicit:
        try:
            # Respect explicit author-specified width for SVG->PPTX fidelity.
            explicit_width = float(str(explicit).replace('px', '').strip())
            return max(1.0, explicit_width)
        except ValueError:
            pass

    slide_w = 1280.0
    if anchor == 'middle':
        available = max(140.0, min(x, slide_w - x) * 2.0 - 20.0)
    elif anchor == 'end':
        available = max(140.0, x - 20.0)
    else:
        available = max(140.0, slide_w - x - 20.0)
    return min(natural_width, available)
...
def convert_text(elem: ET.Element, ctx: ConvertContext) -> str:
    ...
    natural_width = estimate_text_width(full_text, font_size, font_weight) * 1.15
    padding = font_size * 0.1
    max_width = _max_text_width(x, text_anchor, natural_width, elem)
    wrapped_lines = _wrap_runs_by_width(runs, max(80.0, max_width))
    text_width = max(_line_visual_width(line) for line in wrapped_lines) * 1.08
    text_height = max(font_size * 1.35, len(wrapped_lines) * font_size * 1.35)
    ...
    for line_runs in wrapped_lines:
        runs_xml = '\n'.join(_build_run_xml(r, fonts, ctx) for r in line_runs if r.get('text'))
        if not runs_xml:
            runs_xml = '<a:r><a:t> </a:t></a:r>'
...
<a:normAutofit fontScale="92000" lnSpcReduction="12000"/>
</a:bodyPr>
<a:lstStyle/>
{text_body_xml}
</p:txBody>
...
def _validated_image_format(img_data: bytes) -> str:
    """Validate image bytes and return a PPTX-safe canonical extension."""
    if not img_data:
    ...
    try:
        ...
        raise ValueError(f'Image payload is invalid or corrupt: {exc}') from exc
    aliases = {'jpeg': 'jpg', 'jpg': 'jpg', 'png': 'png', 'gif': 'gif', 'webp': 'webp'}
    if detected not in aliases:
        raise ValueError(f'Unsupported image format: {detected or "unknown"}.')
...
def convert_image(elem: ET.Element, ctx: ConvertContext) -> str:
    ...
    if href.startswith('data:'):
        ...
        if not match:
            return ''
        try:
            img_data = base64.b64decode(re.sub(r'\s+', '', match.group(2)), validate=True)
        except Exception as exc:
            raise ValueError(f'Embedded image base64 is invalid: {exc}') from exc
    else:
    ...
    img_format = _validated_image_format(img_data)
